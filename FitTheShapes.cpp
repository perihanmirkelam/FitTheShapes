...
string circlesInTriangle(){
	...
	int row = (svg_h - (circle_r * sqrt(3))) / (diameter -(circle_r*sqrt(3) - circle_r)/4);
	...
	for (int i=0; i<row; i++){
			for(int j=0; j<i+1; j++){
				 svgPart += "<circle cx=\"" + to_string(int((svg_w/2) - (i*circle_r) + (j* diameter)))
						 + "\" cy=\"" +  to_string((int)((i+1)*diameter) - (i * (circle_r*sqrt(3) - circle_r)/4))
...
string rectanglesInTriangle(){
	...
	if(row == 0 && column == 0 && (svg_w / rectangle_h) > 0 && (svg_w / rectangle_w)> 0){
		//if only one and rotated rectangle fit in triangle
		 svgPart += "<rect x=\"" + to_string(svg_w - rectangle_h) + "\" y=\"" + to_string(svg_h - rectangle_w)
				 + "\" width=\"" + to_string(rectangle_h) + "\" height=\"" + to_string(rectangle_w)
	...
	for (int i=0; i<row; i++){
		for(int j=0; j<column; j++){
			if((j+1)*rectangle_w <= upperWidth){
				svgPart += "<rect x=\"" + to_string((i+1)*leapLength + j*rectangle_w)
						+ "\" y=\"" + to_string(bigTriangle_h - rectangle_h)
						+ "\" width=\"" + to_string(rectangle_w) + "\" height=\"" + to_string(rectangle_h)
						+ "\" fill=\"yellow\" style=\"stroke-width:1;stroke:rgb(10,10,10)\" />\n";
				quantityOfSmallShapes++;
			}
		}
...
string circlesInCircle(){
	...
	for (int i=0; i<row; i++){
			for(int j=0; j<i*2+1; j++){
				 svgPart += "<circle cx=\"" + to_string((int)( (svg_w / 2) - ((i * 2* circle_r)) + (j * diameter)))
						 + "\" cy=\"" +  to_string((int)(((i+1)*circle_r) + (i * (circle_r))))
						 + "\" r=\" " + to_string(circle_r)
						 + "\" fill=\"green\" style=\"stroke-width:1;stroke:rgb(10,10,10)\" />\n";
				 svgPart += "<circle cx=\"" + to_string((int)( (svg_w / 2) - ((i *2* circle_r)) + (j * diameter)))
				 						 + "\" cy=\"" +  to_string((svg_h - (int)(((i+1)*circle_r)) - (i * (circle_r))))
				 						 + "\" r=\" " + to_string(circle_r)
				 						 + "\" fill=\"green\" style=\"stroke-width:1;stroke:rgb(10,10,10)\" />\n";
			}
		}
...
string trianglesInRectangle(){
	...
	for (int i=0; i<row; i++){
			for(int j=0; j<column; j++){
				 svgPart += "<polygon points=\""
						 + to_string(j*triangle_e) + "," + to_string(height + i*height) + " " //x1 y1
						 + to_string(triangle_e + (j*triangle_e)) + "," + to_string(height + i*height) + " " //x2 y2
						 + to_string(halfEdge + (j*triangle_e)) + "," + to_string(i*height) //x3 y3
						 +"\" class=\"triangle\" fill=\"green\" style=\"stroke-width:1;stroke:rgb(10,10,10)\" />\n";
				 if((halfEdge + ((j+1)*triangle_e)) <= svg_w){
					 svgPart += "<polygon points=\""
					+ to_string(halfEdge + ((j+1)*triangle_e)) + "," + to_string(i*height) + " " //x1 y1
					+ to_string(triangle_e + (j*triangle_e)) + "," + to_string(height + i*height) + " " //x2 y2
					+ to_string(halfEdge + (j*triangle_e)) + "," + to_string(i*height) //x3 y3
					+"\" class=\"triangle\" fill=\"green\" style=\"stroke-width:1;stroke:rgb(10,10,10)\" />\n";
					 quantityOfSmallShapes++;
...
string circlesInRectangle(){
	...
	for (int i=0; i<row; i++){
			for(int j=0; j<column; j++){
				 svgPart += "<circle cx=\"" + to_string(circle_r + (j*diameter))
...
string rectanglesInRectangle(){
	string svgPart;
	int quantityStraight = (bigRectangle_h / rectangle_h) * (bigRectangle_w / rectangle_w);
	int quantityRotated = (bigRectangle_h / rectangle_w) * (bigRectangle_w / rectangle_h);
	int row=0, column = 0;
	...
	for (int i=0; i<row; i++){
		for(int j=0; j<column; j++){
			 svgPart += "<rect x=\"" + to_string(rectangle_w * j) + "\" y=\"" + to_string(i * rectangle_h)
					 + "\" width=\"" + to_string(rectangle_w) + "\" height=\"" + to_string(rectangle_h)
				    + "\" fill=\"yellow\" style=\"stroke-width:1;stroke:rgb(10,10,10)\" />\n";
		}
	}
...
void drawBigShape(){
	...
	case Shape::T :
		...
		bigTriangle_y3=0;
		bigShapeSvg = "<polygon points=\"" + to_string(bigTriangle_x1) + "," + to_string(bigTriangle_y1)
				+ " " + to_string(bigTriangle_x2) + "," + to_string(bigTriangle_y2) + " "
...
void askRectangleSize(bool isBigShape){
	...
	while(cin.fail() || width < 1){
		cin.clear();
		cin.ignore(1000,'\n');
		cout<<"Bad value! Please enter a positive integer value for width of rectangle: " << endl;
		cin >> width;
	}
	...
	while(cin.fail() || height < 1){
		cin.clear();
		cin.ignore(1000,'\n');
		cout<<"Bad value! Please enter a positive integer value for height of rectangle: " << endl;
		cin >> height;
	}
	if(isBigShape){
...
void askCircleSize(bool isBigShape){
	...
	while(cin.fail() || radius < 1){
		cin.clear();
		cin.ignore(1000,'\n');
		cout<<"Bad value! Please enter a positive integer value for radius of circle: " << endl;
		cin >> radius;
	}
	if(isBigShape){
		bigCircle_r = radius;
...
void askTriangleSize(bool isBigShape){
	...
	while(cin.fail() || edge < 1){
		cin.clear();
		cin.ignore(1000,'\n');
		cout<<"Bad value! Please enter a positive integer value for edge length of triangle: " << endl;
		cin >> edge;
	}
	if(isBigShape){
		bigTriangle_e = edge;
...
void askSmallShapes(){
	string smallShape = "";
	cout << "Please enter the type of shape include your container shape (R, C, T):\n";
	cin >> smallShape;
	...
	case Shape::R :
		...
		cout << "calculateRectangleArea(false): " << calculateRectangleArea(false) << " bigShapeArea: " << bigShapeArea <<endl;
		rectangle_w=0;
		rectangle_h=0;
		rectangleArea = 0.0;
		askSmallShapes();
